...
\\\texttt{airbase-ng start --essid <SSID> -Z 4 -w <Dateiname> wlan0mon}\\
...
\\\texttt{airbase-ng start -P -Z 4 wlan0mon}\\
...
Wir empfehlen im Hintergrund einen \texttt{airodump-ng} Prozess laufen zu lassen, der den Verkehr des Evil-Twins separat aufzeichnet.
Dies hat den Hintergrund, dass die Aufzeichnung von \texttt{airbase-ng} ohne spezifizierte SSID via \texttt{--essid} den Wert \texttt{default} als Netzwerknamen verwendet anstelle des Wertes aus der Probe-Requests.
Ohne gültige SSID werden die Handshakes unbrauchbar, da diese in die Berechnung des PMK mit einfließt.
